...
class _VfsClient:
    ...
    def __init__(self):
        ...
        self._use_telegram = self._config_reader.read_prop(
            "DEFAULT", "use_telegram")
        self._use_twilio = self._config_reader.read_prop(
            "DEFAULT", "use_twilio")
    ...
    def _login(self):
        ...
        _password = self._config_reader.read_prop(
            _section_header, "vfs_password")
        ...
        _email_input = self._web_driver.find_element_by_xpath(
            "//input[@id='mat-input-0']")
        _email_input.send_keys(_email)
        _password_input = self._web_driver.find_element_by_xpath(
    ...
    def _validate_login(self):
        try:
            ...
            if _new_booking_button == None:
                ...
                raise Exception(
                    "Unable to login. VFS website is not responding")
            else:
    ...
    def _get_appointment_date(self, visa_centre, category, sub_category):
        ...
        _zip_input = self._web_driver.find_element_by_xpath(
            "/html/body/div[2]/div[2]/div/div[2]/div[2]/div/div/div/div/div[1]/div/div/div/div[1]/div/label/div[2]/input"
        )
        _zip_input.send_keys("98004")
        time.sleep(5)
        _find_office = self._web_driver.find_element_by_xpath(
            "/html/body/div[2]/div[2]/div/div[2]/div[2]/div/div/div/div/div[1]/div/div/div/button"
        )
        _find_office.click()
        time.sleep(2)
        ...
        try:
            _visa_centre = self._web_driver.find_element_by_xpath(
                "/html/body/div[2]/div[2]/div/div[2]/div[2]/div[2]/div/div/div/div[2]/div/div[2]/button"
            )
        except NoSuchElementException:
            raise Exception("Visa centre not found")
        time.sleep(5)

        # read contents of the text box
        return self._web_driver.find_element_by_xpath("/html/body/div[2]/div[2]/div/div[2]/div[2]/div/div/div/div/div[2]/div/div[2]/div/div[2]/div[1]/button")

    def check_slot(self, visa_centre, category, sub_category):
        self._init_web_driver()

        # open the webpage
        self.vfs_login_url = self._config_reader.read_prop(
            "VFS", "vfs_login_url")
        ...
        _message = self._get_appointment_date(
            visa_centre, category, sub_category)
        logging.debug("Message: " + _message.text)

        if len(_message.text) != 0 and _message.text != "No available appointments":
            logging.info(
                "Appointment slots available: {}".format(_message.text))
            ts = time.time()
